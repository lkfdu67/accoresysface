...
namespace compare{
    Classifier::Classifier(const string& model_file,
        ...
        compare_blob_data_.reset(new CompareTopBlob<float>());
    ...
    const std::vector<Blob<float>* > Classifier::Forward(cv::Mat& im,
        ...
        *input_layer = input_data;
        ...
        if (is_compare){
            compare_blob_data_->SaveInit(saved_folder, net_->net_output_blobs_, net_->layer_names_);

        } else{
            compare_blob_data_->CompareInit(saved_folder, net_->net_output_blobs_, net_->layer_names_);
        }
